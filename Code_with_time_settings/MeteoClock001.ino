...
#define BRIGHT_THRESHOLD_DARK 150 // величина сигнала, ниже которой яркость переключится на минимум (0-1023)
#define BRIGHT_THRESHOLD_LIGHT 180 // величина сигнала, выше которой яркость переключится на максимум (0-1023)
#define LED_BRIGHT_MAX 255 // макс яркость светодиода СО2 (0 - 255)
#define LED_BRIGHT_MIN 32 // мин яркость светодиода СО2 (0 - 255)
...
#define BLUE_YELLOW_WHITE 0 // жёлтый или белый цвет вместо синего (0 - синий, 1 - желтый, 2 - белый) но из за особенностей подключения жёлтый не такой яркий
...
// текущее состояние темно/светло
bool isDark;
...
/// variables to 'clock setup'
GTimer_ms clockTimerSetupScreen(1000);
uint16_t yy, mm, dd, HH, MM;
uint8_t idx;
uint8_t daysPerMonth [2][12] = {{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}, {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}};
bool showFlag;
/// functions to 'clock setup'
void initClockSetup();
...
void setLED(byte color) {
  ...
  switch (color) { // 0 выкл, 1 красный, 2 зелёный, 3 синий (или жёлтый, или белый)
    case 0:
      break;
    case 1:
    ...
    case 2:
      ...
      break;
    case 3:
#if(0 == BLUE_YELLOW_WHITE)
      analogWrite(LED_B, LED_ON);
#else
#if(1 == BLUE_YELLOW_WHITE)
...
void loop() {
  ...
  if (mode == 101)
  {
    if (clockTimerSetupScreen.isReady()) { clockSetupClockTick(); drawClockSetupScreen(false); }// to 'blink' the current parameter
    clockSetupModesTick();
  }
  else
  {
